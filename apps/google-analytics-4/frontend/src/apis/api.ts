...
import { ServiceAccountKeyId, ServiceAccountKey, ContentfulContext, ContentfulContextHeaders, RunReportParamsType } from '../types';
import { fetchFromApi } from 'apis/fetchApi';
import { Headers, ApiErrorType, Credentials, ZRunReportData, ZCredentials, AccountSummaries, ZAccountSummaries, RunReportData } from 'apis/apiTypes';
...
export class Api {
  ...
  constructor(contentfulContext: ContentfulContext, cma: PlainClientAPI, serviceAccountKeyId: ServiceAccountKeyId) {
  ...
  async getServiceAccountKeyFile(): Promise<Credentials> {
    return await fetchFromApi<Credentials>(this.requestUrl('api/service_account_key_file'), ZCredentials, this.contentfulContext.app!, this.cma, {
  ...
  async runReports(params?: RunReportParamsType): Promise<RunReportData> {
    return await fetchFromApi(this.requestUrl('api/run_report', params), ZRunReportData, this.contentfulContext.app!, this.cma, this.serviceAccountKeyHeaders);
  }

  private get serviceAccountKeyHeaders(): Headers {
    return {
      'X-Contentful-ServiceAccountKeyId': this.encodeServiceAccountHeaderValue(this.serviceAccountKeyId),
    };
  }
  ...
  private encodeServiceAccountHeaderValue(headerValue: ServiceAccountKeyId | ServiceAccountKey): string {
