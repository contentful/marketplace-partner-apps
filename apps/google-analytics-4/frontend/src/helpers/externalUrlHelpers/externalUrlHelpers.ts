...
export const getExternalUrl = (
  ...
  const explorerCardSegment = encodeURIComponent(`_u..nav=maui&_r.explorerCard..seldim=["${PAGE_PATH_VIEW}"]`);
  ...
  const dataFiltersSegment = dataFiltersParams
    ? `${encodeURIComponent('&_r..dataFilters')}${encodeDataFiltersParameters(`=[${JSON.stringify(dataFiltersParams)}]`)}`
    : '';

  const dateSegment = dateParams ? encodeURIComponent(`&_u.comparisonOption=disabled&_u.date00=${dateParams.start}&_u.date01=${dateParams.end}`) : '';
...
export const encodeDataFiltersParameters = (str: string) => {
  ...
  const encodedStr = str.replace(/[{}[\]"=]/g, (char) => `%${char.charCodeAt(0).toString(16).toUpperCase()}`);
