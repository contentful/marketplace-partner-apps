...
import { Paragraph, Stack, Subheading, Button, Skeleton, Box, TextLink, Flex } from '@contentful/f36-components';
...
import { AllContentTypes, AllContentTypeEntries, ContentTypeEntries, ContentTypes, ContentTypeValue } from 'types';
...
const AssignContentTypeSection = (props: Props) => {
  const { mergeSdkParameters, onIsValidContentTypeAssignment, parameters, currentEditorInterface, originalContentTypes } = props;
  ...
  const [contentTypeEntries, setContentTypeEntries] = useState<ContentTypeEntries>([] as ContentTypeEntries);
  ...
  const [allContentTypeEntries, setAllContentTypeEntries] = useState<AllContentTypeEntries>([] as AllContentTypeEntries);
  ...
  useEffect(() => {
    ...
    setHasIncompleteContentTypes(Object.entries(contentTypes).some(([contentTypeId]) => !contentTypeId));
  }, [contentTypes]);
  ...
  useEffect(() => {
    const getAllContentTypes = async () => {
      ...
      const formattedContentTypes = sortAndFormatAllContentTypes(contentTypes.items as ContentTypeProps[]);
  ...
  return (
    <Stack spacing="spacingL" flexDirection="column" alignItems="flex-start">
      <Box>
        ...
        <Paragraph>
          Configure content types below that are connected to pages on your website where you're tracking Google Analytics data. You'll need to specify the
          "slug" field used to generate the page path in your website's URL, and optionally a "prefix" if one exists in front of the URL page path. The app will
          automatically be added to the sidebar of associated content types on save of the configuration.
        </Paragraph>
        <Paragraph marginBottom="none">
          <TextLink href="https://www.contentful.com/help/google-analytics-4-app/" target="_blank" icon={<ExternalLinkIcon />} alignIcon="end">
