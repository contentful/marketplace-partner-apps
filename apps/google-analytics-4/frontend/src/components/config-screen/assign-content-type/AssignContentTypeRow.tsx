...
const AssignContentTypeRow = (props: Props) => {
  ...
  useEffect(() => {
    const contentTypeOptions = allContentTypeEntries.filter(([type]) => type === contentTypeId || !contentTypes[type]);
    ...
    if (isSaved) {
      ...
      if (slugField !== undefined) {
        setIsSlugFieldInOptions(allContentTypes[contentTypeId]?.fields.some((field) => field.id === slugField));
      }
    }
  ...
  const validateSelectedOption = (contentTypeId: string, slugField?: string) => {
    ...
    if (slugField !== undefined && allContentTypes[contentTypeId]?.fields.some((field) => field.id === slugField)) {
      value = slugField;
    }
    ...
    <Stack spacing="spacingXs" paddingBottom="spacingS" key={contentTypeId} testId="contentTypeRow">
      ...
      <Box className={styles.contentTypeItem}>
        <Select
          ...
          onChange={(event: React.ChangeEvent<HTMLSelectElement>) => onContentTypeChange(contentTypeId, event.target.value)}
      ...
      <Box className={styles.contentTypeItem}>
        <Select
          ...
          onChange={(event: React.ChangeEvent<HTMLSelectElement>) => onContentTypeFieldChange(contentTypeId, 'slugField', event.target.value)}
      ...
      <Box className={styles.contentTypeItem}>
        <TextInput
          ...
          isDisabled={!contentTypeId || !isContentTypeInOptions}
          onChange={(event: React.ChangeEvent<HTMLInputElement>) => onContentTypeFieldChange(contentTypeId, 'urlPrefix', event.target.value)}
          value={urlPrefix}
        />
      </Box>
