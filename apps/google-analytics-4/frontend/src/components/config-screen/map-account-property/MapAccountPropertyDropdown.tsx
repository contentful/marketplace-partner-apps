...
import { NO_PROPERTIES, getPropertyDeletedMsg, WarningTypes } from 'components/config-screen/WarningDisplay/constants/warningMessages';
...
const MapAccountPropertyDropdown = (props: Props) => {
  const { onSelectionChange, isPropertyIdInOptions, selectedPropertyId, sortedAccountSummaries, originalPropertyId } = props;
  ...
  const selectionValue = selectedPropertyId === originalPropertyId && !isPropertyIdInOptions ? '' : selectedPropertyId;
      ...
      {sortedAccountSummaries.length ? (
        <Stack spacing="spacingXs" flexDirection="column" alignItems="flex-start">
          <Stack spacing="spacingXs">
            ...
            <Select testId="accountPropertyDropdown" value={selectionValue} onChange={onSelectionChange} isInvalid={warningType === WarningTypes.Error}>
              <Select.Option key="empty option" value="" isDisabled>
                Select a property...
              </Select.Option>
              {sortedAccountSummaries.map((accountSummary: AccountSummariesType) => (
                <optgroup label={`${accountSummary.displayName} (${getIdOnly(accountSummary.account)})`} key={accountSummary.account}>
          ...
          <Paragraph marginTop="spacingM" className={styles.helpText}>
            If you don't see a property in the dropdown, make sure the Google service account installed above has been given "viewer" access to it in Google
            Analytics. <em>Note:</em> Only Google Analytics 4 properties are listed; Google "Universal Analytics" properties are not supported. See{' '}
            <TextLink href="https://support.google.com/analytics/answer/10759417" target="_blank" icon={<ExternalLinkIcon />} alignIcon="end">
