...
describe('Setup Google Service Account Details page', () => {
  it('renders account card with no input', () => {
    render(
      <SetupServiceAccountCard parameters={{}} mergeSdkParameters={() => {}} onInEditModeChange={() => {}} isInEditMode={false} onKeyFileUpdate={() => {}} />
    );
  ...
  it('renders an error state when invalid input', async () => {
    ...
    expect(screen.getByLabelText(/Service Account Key/).getAttribute('aria-invalid')).toEqual('true');
