...
import { AssertionError, convertKeyFileToServiceAccountKey, convertServiceAccountKeyToServiceAccountKeyId } from 'utils/serviceAccountKey';
...
export default function SetupServiceAccountCard(props: Props) {
  ...
  const handleKeyFileToServiceAccount = (keyfile: string) => {
    try {
      ...
      const _serviceAccountKeyId = convertServiceAccountKeyToServiceAccountKeyId(_serviceAccountKey);
    ...
    <Stack spacing="spacingL" flexDirection="column">
      <Card>
        <Flex alignItems="center" marginBottom="spacingM" justifyContent="space-between">
          ...
          {isInEditMode && (
            <TextLink testId="cancelServiceAccountButton" as="button" variant="primary" onClick={handleCancelClick}>
              Cancel
            </TextLink>
          )}
        </Flex>
        <Box marginBottom="spacingM">
          <Note variant="primary">
            ...
            to create a service account in Google and obtain the required service account key file (JSON). When you are finished, copy and paste the entire
            contents of this file into the "Service Account Key" field below.
          </Note>
        </Box>
        <FormControl
          ...
          {!rawServiceAccountKey ? (
            <FormControl.HelpText>Paste the service account key file (JSON) above</FormControl.HelpText>
          ) : errorMessage ? (
      ...
      <Paragraph marginBottom="none">
        After configuring the service account, you will create and download a service account key file (JSON) that connects this Contentful space to the Google
        Analytics 4 property you specify.
      </Paragraph>
    </Stack>
  );
}
