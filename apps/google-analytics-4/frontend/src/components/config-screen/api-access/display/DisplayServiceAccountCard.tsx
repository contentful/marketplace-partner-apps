...
import { Badge, Box, Flex, TextLink, Note, Card, Stack, FormControl, Spinner, Button, Skeleton, Text, CopyButton } from '@contentful/f36-components';
...
const DisplayServiceAccountCard = (props: Props) => {
  ...
  useEffect(() => {
    onHasServiceCheckErrorsChange(
      adminApiError || dataApiError || ga4PropertiesError || invalidServiceAccountError || unknownError || !accountSummaries.length
    );
  }, [adminApiError, dataApiError, ga4PropertiesError, onHasServiceCheckErrorsChange, invalidServiceAccountError, unknownError, accountSummaries.length]);

  const handleErrorChanges = useCallback(() => {
    adminApiError || dataApiError || invalidServiceAccountError || ga4PropertiesError ? setShowChecks(true) : setShowChecks(false);
  ...
  const configError = invalidServiceAccountError || missingServiceAccountError || adminApiError || dataApiError || ga4PropertiesError;
  ...
  const RenderStatusInfo = () => {
    if (configError) {
      return isFirstSetup ? <Badge variant="primary">Finish configuration steps below</Badge> : <Badge variant="negative">Problems with configuration</Badge>;
  ...
  return (
    <Card>
      <Flex justifyContent="space-between" marginBottom="spacingS">
        ...
        <Button testId="editServiceAccountButton" onClick={() => onInEditModeChange(true)} variant="secondary" size="small">
          Replace key
        </Button>
      </Flex>
      <FormControl>
      ...
      <FormControl marginBottom="none">
        <FormControl.Label marginBottom="spacing2Xs">
          <Flex alignItems="center">
            ...
            {isLoading ? <Spinner size="small" /> : <CycleIcon size="small" style={{ cursor: 'pointer' }} onClick={handleApiTestClick} />}
          </Flex>
        </FormControl.Label>
        <Box>
          {isLoading ? (
      ...
      {!unknownError && showChecks && !isLoading && (
        <ServiceAccountChecklist
          serviceAccountCheck={{
            ...getServiceKeyChecklistStatus(parameters, invalidServiceAccountError, missingServiceAccountError),
          }}
          adminApiCheck={{
            ...getAdminApiErrorChecklistStatus(isFirstSetup, parameters, invalidServiceAccountError || missingServiceAccountError, adminApiError),
          }}
          dataApiCheck={{
            ...getDataApiErrorChecklistStatus(isFirstSetup, parameters, invalidServiceAccountError || missingServiceAccountError, dataApiError),
          }}
          ga4PropertiesCheck={{
            ...getGa4PropertyErrorChecklistStatus(isFirstSetup, invalidServiceAccountError || missingServiceAccountError, adminApiError, ga4PropertiesError),
          }}
        />
      )}
    </Card>
