...
import { CheckCircleIcon, ClockIcon, ErrorCircleIcon, ArrowForwardIcon } from '@contentful/f36-icons';
...
export const CHECKLIST_STATUSES: ChecklistStatus = {
  ServiceKey: {
    ...
    missing: {
      ...
      title: CHECKLIST_NAMES.serviceAccount,
      description: 'Unable to retrieve your stored service account key. Re-install it if the problem persists',
      disabled: false,
    },
  },
  AdminApi: {
    ...
    invalidServiceAccount: {
      icon: getClockIcon('This check will not run until a valid service account key has been provided'),
      title: CHECKLIST_NAMES.adminApi,
  ...
  GA4Properties: {
    ...
    firstTimeSetup: {
      icon: getArrowIcon(`You need to grant “viewer” access to your service account in a Google Analytics 4 property.`),
      title: CHECKLIST_NAMES.ga4Properties,
    ...
    adminApiError: {
      icon: getClockIcon('This check will not run until the Google Analytics Admin API has been enabled'),
      title: CHECKLIST_NAMES.ga4Properties,
      description: 'Enable Google Analytics Admin API to run this check',
      disabled: true,
    },
    error: {
      icon: getErrorIcon('The service account must have properties assigned to the account for this check to pass'),
      title: CHECKLIST_NAMES.ga4Properties,
      description: "Service account doesn't have access to a Google Analytics 4 property",
      disabled: false,
      checklistUrl: {
  ...
  Other: {
    unknown: {
      icon: getErrorIcon('Something went wrong. Please try again, or get in touch with support of the problem persists'),
...
export const getAdminApiErrorChecklistStatus = (
  ...
  if (!serviceAccountError && !apiError) return { ...CHECKLIST_STATUSES.AdminApi.success, checklistUrl: checklistUrl };
  if (isFirstSetup && (apiError || !serviceAccountError)) return { ...CHECKLIST_STATUSES.AdminApi.firstTimeSetup, checklistUrl: checklistUrl };
...
export const getDataApiErrorChecklistStatus = (
  ...
  if (!serviceAccountError && !apiError) return { ...CHECKLIST_STATUSES.DataApi.success, checklistUrl: checklistUrl };
  if (isFirstSetup && (apiError || !serviceAccountError)) return { ...CHECKLIST_STATUSES.DataApi.firstTimeSetup, checklistUrl: checklistUrl };
...
export const getGa4PropertyErrorChecklistStatus = (
  ...
  if (!serviceAccountError && !adminApiError && !ga4PropertiesError) return CHECKLIST_STATUSES.GA4Properties.success;
  if (isFirstSetup && adminApiError) return CHECKLIST_STATUSES.GA4Properties.firstTimeSetupNotEnabled;
