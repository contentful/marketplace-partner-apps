    ...
    <Box display="flex" style={style}>
      <Flex alignItems="center" justifyContent="space-between" flexGrow={1}>
        <Flex alignItems="center">
          {icon}
          <Text fontColor={disabled ? 'gray500' : 'gray800'} marginRight="spacing2Xs" fontWeight={disabled ? 'fontWeightMedium' : 'fontWeightDemiBold'}>
            {title}
          </Text>
        ...
        {checklistUrl && (
          <Flex paddingRight="spacingS">
            <TextLink icon={<ExternalLinkIcon />} alignIcon="end" href={checklistUrl.url} target="_blank" rel="noopener noreferrer">
              {checklistUrl.title}
            </TextLink>
          </Flex>
        )}
