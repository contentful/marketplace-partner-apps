...
export default function GoogleAnalyticsConfigPage() {
  ...
  const [currentEditorInterface, setCurrentEditorInterface] = useState<Partial<EditorInterface>>({} as Partial<EditorInterface>);
  ...
  const postServiceKeyFileToBackend = useCallback(
    async (validKeyFile: ServiceAccountKey) => {
      ...
      const encodedServiceAccountKeyId = window.btoa(JSON.stringify(convertServiceAccountKeyToServiceAccountKeyId(validKeyFile)));
      ...
      if (!res.ok) {
        sdk.notifier.error('Error: Failed to store Google Service Account Key. Please try again. If the problem persists, please contact support.');
        return false;
      }
  ...
  const handleConfigure = useCallback(async () => {
    ...
    if (parameters.serviceAccountKeyId && isInEditMode && isEmpty(validKeyFile)) {
      sdk.notifier.error('The original service account key will be saved unless a new valid service account key file is provided.');
      setIsSavingConfiguration(false);
      return false;
    }
    ...
    if (parameters.contentTypes) {
      const nonEmptyContentTypes = Object.fromEntries(Object.entries(parameters.contentTypes).filter(([key]) => key !== ''));
    ...
    const newEditorInterfaceAssignments = generateEditorInterfaceAssignments(currentEditorInterface, contentTypeIds, 'sidebar', 1);
  ...
  }, [validKeyFile, isValidAccountProperty, isAppInstalled, isInEditMode, isValidContentTypeAssignment, parameters, currentEditorInterface, sdk.notifier]);
  ...
  const handleConfigurationCompleted = useCallback(async () => {
    ...
    if (!keyFileSaved) {
      sdk.notifier.error('Failed to save private key file. Please try again. Contact support if the problem persists.');
    }
