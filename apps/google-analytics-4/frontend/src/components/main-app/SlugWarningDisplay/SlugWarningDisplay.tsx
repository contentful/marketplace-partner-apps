...
import { SupportHyperLink, AppConfigPageHyperLink } from 'components/main-app/ErrorDisplay/CommonErrorDisplays';
...
const SlugWarningDisplay = (props: Props) => {
  ...
  const { slugFieldIsConfigured, contentTypeHasSlugField, isPublished } = useSidebarSlug(slugFieldInfo);

  const { noSlugConfigMsg, noSlugContentMsg, notPublishedMsg } = getContentTypeSpecificMsg(contentTypeName);
