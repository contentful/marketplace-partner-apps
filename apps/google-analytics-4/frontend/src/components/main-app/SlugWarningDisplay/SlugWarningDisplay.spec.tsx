...
const { noSlugConfigMsg, noSlugContentMsg, notPublishedMsg } = getContentTypeSpecificMsg('Category');
...
describe('SlugWarningDisplay for the analytics app', () => {
  it('mounts with correct msg and hyperlink when error is missing content type config', async () => {
    ...
    const warningMsg = await findByText(noSlugConfigMsg.replace(APP_CONFIG_HYPER_LINK_MSG, '').trim());
  ...
  it('mounts with correct msg and hyperlink when error is missing slug field', async () => {
    ...
    const warningMsg = await findByText(noSlugContentMsg.replace(APP_CONFIG_HYPER_LINK_MSG, '').trim());
    const hyperLink = getByTestId('cf-ui-text-link');
