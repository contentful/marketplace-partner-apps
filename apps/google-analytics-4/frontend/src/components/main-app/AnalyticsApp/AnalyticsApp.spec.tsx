...
import { EMPTY_DATA_MSG, getContentTypeSpecificMsg } from 'components/main-app/constants/noteMessages';
...
const renderAnalyticsApp = async () =>
  render(<AnalyticsApp api={{ runReports: mockApi } as unknown as Api} propertyId="properties/12345" slugFieldInfo={{ slugField: 'title', urlPrefix: '' }} />);
...
describe('AnalyticsApp when content types are not configured correctly', () => {
  it('renders SlugWarningDisplay component when slug field is not configured', async () => {
    ...
    mockApi.mockImplementation(() => runReportResponseHasViews);
    const warningMessage = getContentTypeSpecificMsg('Category').noSlugContentMsg.replace('app configuration page.', '').trim();
    renderAnalyticsApp();
