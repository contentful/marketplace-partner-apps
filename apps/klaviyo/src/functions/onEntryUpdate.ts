...
async function resolveAssetReferences(sdk: SidebarExtensionSDK, entry: any, mappings: FieldMapping[]) {
  try {
...
function extractContentFromEntry(entry: any, targetFieldId?: string): string {
  try {
    ...
    if (targetFieldId && entry.fields && entry.fields[targetFieldId]) {
      ...
      const fieldValue = entry.fields[targetFieldId]['en-US'] || 
    ...
    for (const commonField of commonFields) {
      if (entry.fields && entry.fields[commonField]) {
        const value = entry.fields[commonField]['en-US'] || 
...
function getEntryIdsFromArray(value: string): string[] {
  try {
    ...
    return parsed
      .filter(item => item?.sys?.type === 'Link' && item?.sys?.linkType === 'Entry' && item?.sys?.id)
...
async function resolveEntryReferenceArray(sdk: SidebarExtensionSDK, value: string | any[]): Promise<string> {
  try {
    let entryIds: string[] = [];
...
function richTextToHtml(richTextNode: any): string {
  ...
  try {
    ...
    if (richTextNode.nodeType === 'hyperlink') {
      const content = richTextNode.content?.map((node: any) => richTextToHtml(node)).join('') || '';
      return `<a href="${richTextNode.data?.uri || '#'}" ${richTextNode.data?.title ? `title="${richTextNode.data.title}"` : ''}>${content}</a>`;
    }
    ...
    if (richTextNode.nodeType === 'embedded-entry-block' || 
...
export async function onEntryUpdate(event: EntryEventData) {
  ...
  console.log("Processing mappings:", mappings.map(m => ({
  ...
  const klaviyoConfig = {
    apiKey: parameters?.klaviyoApiKey as string,
    companyId: parameters?.klaviyoCompanyId as string,
  };
  ...
  try {
    ...
    for (const mapping of mappings) {
      ...
      // For entry field type, which is specifically for entry references
        ...
        try {
          ...
          // 2. Entry Link
          else if (value && value.sys && value.sys.type === 'Link' && value.sys.linkType === 'Entry') {
      ...
      // Text fields
        ...
        try {
          ...
          console.log(`Text field ${fieldId} value:`, textValue);
          ...
          if (isRichTextDocument(textValue)) {
            ...
            console.log(`Converted rich text to HTML: ${htmlContent.substring(0, 100)}${htmlContent.length > 100 ? '...' : ''}`);
          ...
          if (field?._fieldLocales?.['en-US']?._value && 
          ...
          if (typeof textValue === 'string' && textValue.includes('"sys"') && textValue.includes('"linkType":"Entry"')) {
            // Check if it's an array of references
            ...
            try {
              ...
              if (parsed.sys && parsed.sys.type === 'Link' && parsed.sys.linkType === 'Entry' && parsed.sys.id) {
    ...
    const klaviyoService = new KlaviyoService(klaviyoConfig);
