...
describe('klaviyo-api-service', () => {
  ...
  describe('sendToKlaviyo', () => {
    it('should throw error if API key is missing', async () => {
      ...
      const entryData = {
        email: { id: 'email', name: 'Email', type: 'Symbol', value: 'test@example.com', isAsset: false }
      };

      // Act & Assert
      await expect(sendToKlaviyo(config, mappings, entryData)).rejects.toThrow('Klaviyo API key is required');
    });
    ...
    it('should throw error if email or phone number is missing', async () => {
      ...
      // Act & Assert
      await expect(sendToKlaviyo(config, mappings, entryData)).rejects.toThrow('Either email or phone number is required for Klaviyo profiles');
    });
    ...
    it('should send data to Klaviyo API successfully', async () => {
      ...
      // Assert
      expect(global.fetch).toHaveBeenCalledWith('https://a.klaviyo.com/api/v2/profiles', expect.any(Object));
  ...
  describe('SyncContent', () => {
    it('should throw error if API key is missing', async () => {
      ...
      // Act & Assert
      await expect(syncContent.syncContent(mockSdk, mappings)).rejects.toThrow('Klaviyo API key is missing');
    });
    ...
    it('should process fields and call API', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith('/api/klaviyo', expect.objectContaining({
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        }
      ...
      expect(mockSdk.notifier.success).toHaveBeenCalledWith('Content synced to Klaviyo successfully');
    ...
    it('should handle errors during sync', async () => {
      ...
      // Act & Assert
      await expect(syncContent.syncContent(mockSdk, mappings)).rejects.toThrow(/Klaviyo sync error/);
