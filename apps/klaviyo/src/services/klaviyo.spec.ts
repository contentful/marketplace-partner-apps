...
describe('KlaviyoService', () => {
  ...
  describe('createUniversalContentBlock', () => {
    ...
    it('handles JSON content properly', async () => {
      ...
      // Assert
      expect(mockProxyApi.post).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({
  ...
  describe('syncContent', () => {
    ...
    it('processes image fields correctly', async () => {
      ...
      (service as any).getAssetUrl = vi.fn().mockReturnValue('https://images.ctfassets.net/image.jpg');
      service.uploadImage = vi.fn().mockResolvedValue({ id: 'image123', imageUrl: 'https://klaviyo.com/image123.jpg' });
      ...
      expect(service.uploadImage).toHaveBeenCalledWith('https://images.ctfassets.net/image.jpg', 'Image');
    ...
    it('handles JSON objects appropriately', async () => {
      ...
      (service as any).formatJsonObject = vi.fn().mockReturnValue('name: Test\nage: 30\nactive: true');
      service.createUniversalContentBlock = vi.fn().mockResolvedValue({ id: 'block123' });
      ...
      expect(service.createUniversalContentBlock).toHaveBeenCalledWith('JSON Data', 'name: Test\nage: 30\nactive: true');
      expect(result).toEqual([{ id: 'block123' }]);
    });
