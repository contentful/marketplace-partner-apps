...
const filterUnsupportedFields = (fields: FieldOption[]): FieldOption[] => {
  return fields.filter(field => {
    ...
    if ((field.type && field.type === 'Array' && field.name.toLowerCase().includes('reference')) || 
  ...
  return (
    <Box padding="spacingM">
      <Stack spacing="spacingS" flexDirection='column' alignItems="flex-start">
        ...
        {fields.length > 0 ? (
          <Box width="100%">
            <Stack flexDirection='column' spacing="spacingXs">
              {fields.map(field => (
                <FormControl key={field.id} marginBottom="none" style={{ textAlign: 'left', width: '100%' }}>
                  <Checkbox
                    id={field.id}
                    name={field.id}
        ...
        <Stack spacing="spacingS" justifyContent="flex-start" flexDirection="row" marginTop="spacingM">
          <Button 
            variant="primary" 
