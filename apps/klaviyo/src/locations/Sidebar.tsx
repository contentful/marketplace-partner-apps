...
const ConfigComponent = ({
  ...
  const handleSave = useCallback(async (values: FieldData[]) => {
    setLoading(true);
    setMappings(values);
...
const SidebarComponent = ({
  ...
  const handleGenerateClick = useCallback(async () => {
    ...
    try {
      ...
      const contentType = await sdk.space.getContentType(
        sdk.ids.contentType
      );
  ...
  const handleRemoveMapping = useCallback(async (indexToRemove: number) => {
  ...
  return (
    <Flex flexDirection="column" gap="spacingM" style={{ maxWidth: '300px' }}>
      ...
      {mappings.length > 0 ? (
        <Flex flexDirection="column" gap="spacingXs" padding="spacingS" style={{ background: '#F7F9FA', borderRadius: '4px' }}>
          <Text fontWeight="fontWeightMedium">
            {mappings.length} Field{mappings.length > 1 ? 's' : ''} Ready to Sync:
          </Text>
          {mappings.map((mapping, index) => (
            <Flex key={`${mapping.id}-${index}`} justifyContent="space-between" alignItems="center" padding="spacingXs" marginBottom="spacingXs" style={{ borderBottom: '1px solid #EEEEEE' }}>
              <Text fontSize="fontSizeS">{mapping.name}</Text>
              <Flex alignItems="center" gap="spacingXs">
                <Text fontSize="fontSizeS" fontColor="gray600">{mapping.isAsset ? 'Image' : 'Text'}</Text>
                <Button 
                  size="small" 
                  variant="transparent" 
        ...
        <Flex justifyContent="center" padding="spacingS">
          <Text fontColor="gray600" fontSize="fontSizeS">No fields configured yet. Click "Configure Field Mappings" to get started.</Text>
        </Flex>
      )}
      ...
      {showSuccess && (
        <Text fontColor="colorPositive">Successfully synced to Klaviyo!</Text>
      )}
    </Flex>
  );
};
