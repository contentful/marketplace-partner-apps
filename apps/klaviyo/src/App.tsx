import { AppExtensionSDK, locations, SidebarExtensionSDK, DialogExtensionSDK } from '@contentful/app-sdk';
...
const App = () => {
  ...
  // Initialize the app - load mappings and set up event handlers
  useEffect(() => {
    ...
    const loadMappings = async () => {
      try {
        if (sdk.location.is(locations.LOCATION_ENTRY_SIDEBAR) || 
          ...
          setMappings(config?.mappings as FieldMapping[] || []);
        }
      } catch (error) {
  ...
  const getCurrentComponent = () => {
    ...
    for (const [location, Component] of Object.entries(ComponentLocationSettings)) {
      if (sdk.location.is(location)) {
        ...
        const needsMappings = location === locations.LOCATION_ENTRY_SIDEBAR || 
