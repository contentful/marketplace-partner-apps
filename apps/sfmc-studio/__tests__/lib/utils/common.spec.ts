import { formatInput, defaultSystemTZ, clientCredsCookieName, CookieHelpers, getDateRange, encryptData, decryptClientData } from '@/lib/utils/common';
...
describe('Utility Functions', () => {
  ...
  describe('CookieHelpers', () => {
    ...
    describe('setCookie', () => {
      it('should set the cookie with the specified value and duration', () => {
        ...
        expect(document.cookie).toBe(`${name}=${value}${expires}; path=/; SameSite=None; Secure`);
      });
    });
    ...
    describe('deleteCookie', () => {
      it('should delete the specified cookie', () => {
        ...
        expect(document.cookie).toBe('testCookie=; Max-Age=-99999999; path=/; SameSite=None; Secure');
      });
    });
  });
  ...
  test('decryptClientData handles errors', () => {
    expect(() => decryptClientData('invalid-encrypted-data')).toThrow('unable to parse req');
  });
  ...
  describe('getDateRange', () => {
    ...
    test('returns correct date range for "1 month"', () => {
      ...
      const now = dayjs.utc();
      const expectedStartDate = now.subtract(1, 'month').startOf('day').toDate();
      const expectedEndDate = now.endOf('day').toDate();
    ...
    test('returns correct date range for "3 months"', () => {
      ...
      const now = dayjs.utc();
      const expectedStartDate = now.subtract(3, 'months').startOf('day').toDate();
      const expectedEndDate = now.endOf('day').toDate();
    ...
    test('returns correct date range for "6 months"', () => {
      ...
      const now = dayjs.utc();
      const expectedStartDate = now.subtract(6, 'months').startOf('day').toDate();
      const expectedEndDate = now.endOf('day').toDate();
