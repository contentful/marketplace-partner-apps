    ...
    return (
      <div>
        ...
        <div data-testid="axis-test">
          Y Axis Title: {axis.y.title}X Axis Label Spacing: {axis.x.labelSpacing}
        </div>
      </div>
    );
  }),
...
describe('BarChart', () => {
    ...
    expect(barConfig).toMatchObject({
      ...
      axis: expect.objectContaining({
        y: expect.objectContaining({
          ...
          titleFill: themeTextColor[initialState.themeSlice.theme as keyof typeof themeTextColor],
          titleStroke: themeTextColor[initialState.themeSlice.theme as keyof typeof themeTextColor],
          style: {
          ...
          labelFontSize: 14,
          labelFontWeight: 500,
          labelFill: themeTextColor[initialState.themeSlice.theme as keyof typeof themeTextColor],
          labelOpacity: 1,
  ...
  it('calculates dx correctly based on yField value', () => {
    ...
    expect(dxFunction(smallValue, 0, data)).toBe(getDxForOutsideLbl(smallValue.y));
  });
  ...
  it('calculates fill color correctly based on theme and yField value', () => {
    ...
    expect(fillFunction(defaultProps.data[0], 0, defaultProps.data)).toBe('red');
    ...
    expect(darkFillFunction(defaultProps.data[0], 0, defaultProps.data)).toBe('red');
  });
  ...
  it('checks the axis configuration', () => {
    ...
    expect(axis.y.titleFill).toBe(themeTextColor[initialState.themeSlice.theme as keyof typeof themeTextColor]);
    expect(axis.y.titleStroke).toBe(themeTextColor[initialState.themeSlice.theme as keyof typeof themeTextColor]);
    expect(axis.y.labelFontSize).toBe(14);
    expect(axis.y.labelFontWeight).toBe(500);
    expect(axis.y.labelFill).toBe(themeTextColor[initialState.themeSlice.theme as keyof typeof themeTextColor]);
  ...
  it('formats the tooltip text correctly', () => {
    ...
    const tooltipText = tooltipFunction(defaultProps.data[0], 0, defaultProps.data);
  ...
  it('formats the axis labels correctly', () => {
    ...
    expect(xAxisFormatter(exampleText)).toBe(commonChartConfig.transformLegendText(exampleText, 15));
  });
});
