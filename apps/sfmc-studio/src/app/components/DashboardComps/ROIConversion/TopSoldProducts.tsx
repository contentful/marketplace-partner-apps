...
function TopSoldProducts({ soldProduct }: { soldProduct: any }) {
  ...
  let legend = {
    ...
    itemLabelText: (d: any) => commonChartConfig.transformLegendText(d?.label, 20),
    mouseenter: (e: any) => {
    ...
    <>
      <div className={style.TopSoldMain}>
        <div className={`CanvesData ${style.TopSoldInner} ${theme}`}>
          ...
          {soldProduct?.length ? <MultiLineChart data={soldProduct} yField="revenue" colorField="category" legend={legend} /> : ''}
