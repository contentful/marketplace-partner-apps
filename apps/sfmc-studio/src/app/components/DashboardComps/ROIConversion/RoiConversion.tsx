...
function RoiConversion({ order }: { order: number }) {
  ...
  const [roiConversionCounts, setRoiConversionCounts] = useState<
    RoiConversionCounts[]
  >([
    {
    ...
    },
    {
      ...
      toolTipText:
        "The aggregate amount of income generated from all sales or transactions.",
    },
    {
  ...
  const [sourceRevenue, setSourceRevenue] = useState<
    TopSourceMediumRevenueType[]
  >([]);
  const [revenueCity, setRevenueCity] = useState<TopRevenueCityType[]>([]);
  const [topRevenueOrderType, setTopRevenueOrderType] = useState<
    TopRevenueOrderT[]
  >([]);
  ...
  const fetchData = async (dateRange: dateStartEnd) => {
    if (parameters?.installation?.licenseKey && isAuth) {
      try {
        ...
        setRoiConversionCounts([
          ...
          },
          {
            ...
            currencySign: getSymbolFromCurrency(
              conversionCountRes?.data?.data?.totalRevenue.currency
            ),
        ...
        setSourceRevenue(
          topSourceRes?.data?.data.map(
            (elm: TopSourceMediumRevenueType, i: number) => {
              return {
                ...elm,
        ...
        setRevenueCity(
          topRevenueRes?.data?.data?.map(
            (elm: TopRevenueCityType, i: number) => {
              return {
                ...elm,
