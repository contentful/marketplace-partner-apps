...
export default function TopProductFamily({ topProductFamily }: { topProductFamily: any }) {
  ...
  let legend = {
    ...
    itemMarker: 'circle',
    itemMarkerFill: (d: any[], index: number, data: any[]) => multiLineChart[index],
    itemLabelText: (d: any) => commonChartConfig.transformLegendText(d?.label, 10),
    mouseenter: (e: any) => {
  ...
  return (
    <div className={`${style.TopProductFamilyMain} ${theme == 'dark' ? style.DarkTheme : ''}`}>
      <div className={`CanvesData ${style.TopProductFamilyInner} ${theme}`}>
        <h4>Top Product Family</h4>
        {topProductFamily?.length ? <MultiLineChart data={topProductFamily} yField="revenue" colorField="family" legend={legend} /> : <NoData />}
        {textHover !== '' && <div className={`${theme == 'dark' ? 'LegendHover-dark' : 'LegendHover'} `}>{textHover}</div>}
      </div>
    </div>
  );
}
