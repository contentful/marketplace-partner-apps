...
function OrderByStatus({ orderStatus }: { orderStatus: any }) {
  ...
  let tooltip = (d: any, index: number, data: any[]) => ({
    ...
    value: `${formatInput(data[index]['count'], data?.[index]?.CurrencyIsoCode)}`,
    name: data[index]['status'],
  });

  return (
    <div className={`${style.OrderStatusMain} ${theme == 'dark' ? style.DarkTheme : ''}`}>
      <div className={`${style.OrderStatusInner} ${theme}`}>
        ...
        <div className={style.CanvesDataGrapInner}>
          <div className={style.CanvesGrapInner}>
            {orderStatus?.length ? (
              <PieChartWithRadius data={orderStatus} angleField="count" colorField="status" innerRadius={0.6} width={280} height={300} />
            ) : (
              ''
            )}
          </div>
