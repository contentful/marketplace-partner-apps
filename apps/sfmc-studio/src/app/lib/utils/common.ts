...
export const saveOrValidateLicenseKey = async (licenseKey: string, spaceId: string, client: AxiosInstance) => {
  return client.post(
...
export const customDateRange = ['24 hours', '3 days', '7 days', '1 month', '3 month', '6 month', 'Set Custom'];
...
export const encryptData = (data: any) => {
  try {
    ...
    const encrypted = CryptoJS.AES.encrypt(dataToEncrypt, CRYPT_SECRET_KEY).toString();
    return encrypted;
  } catch (e) {
...
export const decryptClientData = (encryptedData: string) => {
  try {
    const decrypted = CryptoJS.AES.decrypt(encryptedData, CRYPT_SECRET_KEY).toString(CryptoJS.enc.Utf8);
