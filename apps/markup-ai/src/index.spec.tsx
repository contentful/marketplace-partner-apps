...
vi.mock("./components/LocalhostWarning/LocalhostWarning", () => ({
  default: () => <div>LocalhostWarning</div>,
}));
...
describe("index.tsx", () => {
  ...
  it("renders app when not in top window or not development", async () => {
    ...
    const __renderMock = (
      client as unknown as { __renderMock: { mock: { calls: Array<unknown[]> } } }
    ).__renderMock;
    await import("./index");
  ...
  it("renders localhost warning in development top window", async () => {
    ...
    if (!root) throw new Error("Root element not found in test");
    root.innerHTML = "";
