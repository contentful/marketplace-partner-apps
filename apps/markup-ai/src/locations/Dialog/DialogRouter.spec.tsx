...
describe('DialogRouter', () => {
  it('renders Rewrite Dialog when startRewrite param is present', () => {
    mockUseSDKReturn = { parameters: { invocation: { startRewrite: true } } } as unknown as Partial<DialogAppSDK>;
    render(<DialogRouter />);
  ...
  it('renders fallback for unknown dialog type', () => {
    mockUseSDKReturn = { parameters: { invocation: { foo: 'bar' } } } as unknown as Partial<DialogAppSDK>;
    render(<DialogRouter />);
