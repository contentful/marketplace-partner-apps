...
class WingifyAppActionService {
  ...
  async initialize() {
    try {
      ...
      const appAction =
        appActions.items.find((action) => action.sys.id === globalConstants.WINGIFY_APP_ACTION_ID) ||
        appActions.items.find((action) => action.name === globalConstants.WINGIFY_APP_ACTION_NAME);

      if (!appAction) {
        throw new Error('Wingify app action not found. Deploy the latest app bundle and run `npm run upsert-actions` to register app actions.');
      }

      this.actionId = appAction.sys.id;
    } catch (err) {
      throw new Error(`Failed to initialize WingifyAppActionService: ${err.message}`);
    }
  }

  /**
   * Fetch feature flag details from Wingify
  ...
  /**
   * Create a new feature flag in Wingify
  ...
  /**
   * Update feature flag details in Wingify
  ...
  /**
   * Update variations in Wingify
   * @param {Array} wingifyVariations - Array of variations to update
   * @returns {Promise<Array>} - Updated variations array
   */
  async updateVariations(wingifyVariations, featureId) {
    try {
      if (!this.actionId) {
        await this.initialize();
      }
      ...
      const filteredWingifyVariations = wingifyVariations.filter((variation) => variation.id !== 1);
      const { response } = await this.cma.appActionCall.createWithResponse(
        {
          appActionId: this.actionId,
          appDefinitionId: this.sdk.ids.app,
        ...
        },
        {
          parameters: {
            ...
            payload: JSON.stringify({ variations: filteredWingifyVariations, featureId }),
          },
        }
      );
...
export default WingifyAppActionService;
