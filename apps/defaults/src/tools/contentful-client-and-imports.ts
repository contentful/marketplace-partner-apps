...
console.log("CONTENTFUL_ORG_ID:", process.env.CONTENTFUL_ORG_ID);
console.log("CONTENTFUL_ACCESS_TOKEN:", process.env.CONTENTFUL_ACCESS_TOKEN);
console.log("CONTENTFUL_APP_DEF_ID:", process.env.CONTENTFUL_APP_DEF_ID);
...
assert.ok(
  spaceId !== "",
  `CONTENTFUL_SPACE_ID environment variable must be defined`
);

assert.ok(
  functionId !== "",
  `CONTENTFUL_FUNCTION_ID environment variable must be defined`
);
