...
import { CalendarFilter, CalendarFilterCondition, MultiSelectFilter, SearchBar as SearchBarComponent, SelectFilter } from '../../../../components';
import { useCallback, useEffect, useState } from 'react';

export const SearchBar = ({ isDisabled = false, search, onSearch, contentTypeOptions, userOptions, onFilterChange }: SearchBarProps) => {
  ...
  const filterMenuItems = Object.entries(filterProps).map(([filterName, { props, toggle }]) => (
    <Menu.Item key={filterName} icon={isActive(filterName as FilterName) ? <DoneIcon /> : undefined} onClick={toggle}>
      {props.name}
    </Menu.Item>
  ));
