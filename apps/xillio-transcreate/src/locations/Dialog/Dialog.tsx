...
export type DialogInvocationParameters = CalendarParameters | ConfirmDialogParameters | TranslateDialogParameters | UpdateDialogParameters;
...
export const Dialog = () => {
  ...
  if (sdk.parameters.invocation.type === 'calendar') {
    const selected = sdk.parameters.invocation.initialDate ? new Date(sdk.parameters.invocation.initialDate) : undefined;

    return (
      <Calendar css={css({ padding: tokens.spacingM })} mode="single" selected={selected} onSelect={sdk.close} fromDate={new Date(Date.now() + 86400000)} />
    );
  }
  ...
  if (sdk.parameters.invocation.type === 'update') {
    return <UpdateDialog defaultValues={{ dueDate: new Date(sdk.parameters.invocation.dueDate) }} onClose={sdk.close} />;
  }
};

type OpenCurrentAppOptions = Omit<OpenCustomWidgetOptions, 'id' | 'shouldCloseOnOverlayClick' | 'shouldCloseOnEscapePress' | 'parameters'>;
