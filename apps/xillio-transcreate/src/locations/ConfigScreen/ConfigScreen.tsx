...
import { Button, Checkbox, Flex, Heading, ModalLauncher, Paragraph, TextLink } from '@contentful/f36-components';
...
import { ControlledCheckbox, ControlledSelect, ControlledTextInput, SelectOption, Divider, DocsLink } from '../../components';
...
export const ConfigScreenComponent = ({
  ...
  return (
    <div css={css({ background, position: 'fixed', inset: 0, overflow: 'auto' })}>
      <Flex justifyContent="center" alignItems="flex-start" padding="spacingXl" css={css({ minHeight: '100%' })}>
        <Flex
          ...
          <Section
            ...
            <Flex flexDirection="column" gap="spacingS" paddingTop="spacingL" paddingBottom="spacingL" alignSelf="center" css={css({ width: 'fit-content' })}>
              ...
              <Paragraph
                ...
                {connected ? <CheckCircleIcon variant="muted" /> : <LockIcon variant="muted" />}
                {connected ? (
                  'Succesfully connected to account'
                ) : (
                  <span>
                    Don't have an account?{' '}
                    <TextLink href="https://www.xillio.com/transcreate#form" target="_blank">
                      Contact us.
                    </TextLink>
                  </span>
                )}
          ...
          <Section title="Configure defaults" subtitle={`Requires a ${appConfig.name} account`} isDisabled={isDisabled}>
            <ControlledSelect
            ...
            <ControlledTextInput
              ...
              isRequired
            />
            <ControlledCheckbox disabled={isDisabled} control={control} label="Send all references recursively with entry" name="sendRecursively" />
          </Section>

          {contentTypes && contentTypes.length > 0 && (
            <>
              <Divider />
              <Section title="Add to sidebar views" subtitle={`Assign ${appConfig.name} to content types`} isDisabled={isDisabled}>
                <Flex flexDirection="column" gap="spacingS">
                  {contentTypes.length > 1 && (
                    <Checkbox isDisabled={isDisabled} isChecked={selectedContentTypes.size === contentTypes?.length} onChange={toggleAllSelectedContentTypes}>
                  ...
                  <Flex flexDirection="column" gap="spacingS" paddingLeft="spacingS">
                    {contentTypes.map((contentType) => (
                      <Checkbox
                        ...
                        onChange={() => toggleSelectedContentTypes(contentType.sys.id)}>
                        {contentType.name}
                      </Checkbox>
                    ))}
                  </Flex>
...
export const ConfigScreen = () => {
  ...
  const { selected, setSelected, toggleSelected, toggleAllSelected } = useSelected(contentTypes?.map((contentType) => contentType.sys.id));
  ...
  const handleConfigurationCompleted = useCallback(async () => {
    ...
    const updates = editorInterfaces.map(async (editorInterface) => {
      ...
      const isInstalled = Boolean(editorInterface.sidebar?.find((sidebarItem) => sidebarItem.widgetId === sdk.ids.app));
      if (isSelected && !isInstalled) {
        ...
        await sdk.cma.editorInterface.update(
          ...
          {
            ...editorInterface,
            sidebar: [{ settings: {}, widgetId: sdk.ids.app, widgetNamespace: 'app' }, ...sidebar],
          },
        );
        ...
        await sdk.cma.editorInterface.update(
          ...
          {
            ...editorInterface,
            sidebar: editorInterface.sidebar?.filter((sidebarItem) => sidebarItem.widgetId !== sdk.ids.app),
          },
        );
      }
    });
  ...
  useEffect(() => {
    ...
    api.projects
      ...
      .catch(() => {
        sdk.notifier.error(`Failed to load available projects from your ${appConfig.name} account. Please check the app configuration.`);
      });
  }, [token]);
  ...
  useEffect(() => {
    ...
    const contentTypeIds = editorInterfaces
      .filter((editorInterface) => editorInterface.sidebar?.find((sidebarItem) => sidebarItem.widgetId === sdk.ids.app))
    ...
    (async () => {
      ...
      if (currentParameters) {
        for (const name in currentParameters) {
          setValue(name as keyof AppInstallationParameters, currentParameters[name as keyof AppInstallationParameters]);
        }
      }
