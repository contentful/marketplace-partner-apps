...
export const ControlledMultiSelect = <T extends FieldValues>({
  ...
  const unselected = useMemo(() => Object.keys(options).filter((value) => !selected.includes(value)), [selected, options]);
      ...
      <Flex flexWrap="wrap" gap="spacingXs" css={css({ minHeight: 40 })}>
        {Boolean(unselected.length) && (
          <Menu isAutoalignmentEnabled={isAutoalignmentEnabled} placement={placement}>
            <Menu.Trigger>
              <IconButton variant="secondary" icon={<PlusIcon />} aria-label="add locale" isDisabled={disabled} />
            </Menu.Trigger>
