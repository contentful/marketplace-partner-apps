...
export const MultiSelectFilter = <MultiSelectFilterValue extends string>({
  ...
  const hasChanges = useMemo(
    () => ![...currentSelected].every((id) => selected.has(id)) || ![...selected].every((id) => currentSelected.has(id)),
  ...
  return (
    <Popover isOpen={isOpen} onClose={() => setIsOpen(false)}>
      <Filter name={name} value={value} Trigger={Popover.Trigger} onClick={() => setIsOpen(!isOpen)} isDisabled={isDisabled} />
        ...
        {hasSelectedOptions || hasUnselectedOptions ? (
          <>
            <div
              ...
              {hasSelectedOptions && <MultiSelectFilterOptionList options={selectedOptions} selected={selected} onSelect={setSelected} />}
              {hasSelectedOptions && hasUnselectedOptions && <Divider />}
              {hasUnselectedOptions && <MultiSelectFilterOptionList options={unselectedOptions} selected={selected} onSelect={setSelected} />}
            </div>
            <Divider />
            <Flex padding="spacingXs" gap="spacingXs">
              <Button variant="primary" css={css({ flexGrow: 1 })} isDisabled={!hasChanges} onClick={handleApply}>
                Apply
              </Button>
              <Button variant="transparent" css={css({ flexGrow: 1 })} isDisabled={!currentSelected.size} onClick={handleReset}>
                Reset to Any
              </Button>
            </Flex>
          </>
  ...
  return (
    <Flex padding="spacingXs" gap="spacingXs" flexDirection="column">
      {Object.keys(options).map((id) => (
        <Checkbox key={id} isChecked={selected.has(id as MultiSelectFilterValue)} onChange={() => handleSelect(id as MultiSelectFilterValue)}>
