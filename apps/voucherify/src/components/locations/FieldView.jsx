...
const FieldView = ({ resourceType }) => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const linkedResources = await fetchLinkedResources({
          entryId,
          page: page + 1,
          appId,
          secretKey,
  ...
  return (
    <Stack flexDirection="column" alignItems="start" style={{ padding: '1rem', border: '1px #eee solid', borderRadius: '7px', marginBottom: '2rem' }}>
      {resourcesLinkedToEntry?.length > 0 && (
        <Button startIcon={<SearchIcon />} size="small" onClick={toggleVisibilityOfSearchComponent}>
          Search linked {resourceDisplayedName}
        </Button>
      )}
            ...
            {resourcesLinkedToEntry.map((resource) => (
              <Table.Row key={resource.id}>
                ...
                <Table.Cell style={{ textAlign: 'end', paddingRight: '25px' }}>
                  <Button size="small" variant="negative" onClick={unlinkResource(resource.id)}>
                    Remove
                  </Button>
                </Table.Cell>
              </Table.Row>
      ...
      {resourcesLinkedToEntry?.length > 0 && <Pagination activePage={page} totalItems={totalItems} itemsPerPage={ITEMS_PER_PAGE} onPageChange={setPage} />}
