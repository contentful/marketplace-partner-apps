...
const DialogView = () => {
    ...
    useEffect(() => {
        (async () => {
            try {
                const unlinkedResources = await fetchUnlinkedResources({ entryId, page: page + 1, appId, secretKey, customURL, searchQuery: searchedResourceName, resourceType });
    ...
    return (
        <Flex flexDirection="column">
            <Modal.Header
                title="Link new resource"
                onClose={() => sdk.close({ action: "cancel" })}
            />
            ...
            <Modal.Content style={{ minHeight: "400px", minWidth: "500px" }}>
                ...
                {currentItems.length > 0 ?
                    ...
                    >
                        {currentItems.map(resource => (
                            <Radio
                                key={resource.id}
                                value={resource.id}
                                isDisabled={!resource.isEditable}
                            >
                                <span>{resource.name ? resource.name : resource.id} {!resource.isEditable && "(Locked)"}</span>
                                <TextLink
                    ...
                    </Radio.Group> : <Text fontSize="fontSizeL">No resources found</Text>}
            </Modal.Content>
            <Modal.Controls>
                {currentItems.length > 0 &&
                    <Pagination
                ...
                <Button
                    size="small"
                    variant="transparent"
                    onClick={() => sdk.close({ action: "cancel" })}
                >
                    Close
                </Button>
                <Button
