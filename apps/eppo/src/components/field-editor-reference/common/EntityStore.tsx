  ...
  const [resource, resourceTypes] = await Promise.all([
    fetch(
      ...
      ({ cmaClient }): Promise<ExternalResource | null> =>
        cmaClient.raw
          .get<
            CollectionResponse<ExternalResource>
          >(`/spaces/${spaceId}/environments/${environmentId}/resource_types/${resourceType}/resources`, { params: { 'sys.urn[in]': urn } })
  ...
  function useInitServices(props: EntityStoreProps) {
    ...
    const getEntity = useCallback(
      function getEntity<E extends FetchableEntity>(
        ...
        return fetch(
          queryKey,
          // @ts-expect-error external code from contentful
          ({ cmaClient }) => {
            if (entityType === 'Entry') {
    ...
    const isSameSpaceEntityQueryKey = useCallback(
      (queryKey: QueryKey) => {
        ...
        return (
          isEntityKey &&
          queryKey[2] === currentSpaceId &&
          environmentIds.includes(queryKey[3] as string)
        );
      },
