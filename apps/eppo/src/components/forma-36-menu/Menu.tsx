...
import { Popover, PopoverProps } from '../forma-36-popover';
...
export function Menu(props: MenuProps) {
  ...
  const { isOpen, handleOpen, handleClose, isControlled } = useControllableState({
  ...
  const { focusedIndex, handleArrowsKeyDown, setFocusedIndex } = useArrowKeyNavigation({
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
  ...
  useEffect(() => {
    if (isOpen && menuListRef.current) {
      const menuItems = Array.from(menuListRef.current.querySelectorAll(MENU_ITEMS_SELECTOR));
  ...
  const focusMenuItem = useCallback(
    (item: HTMLElement) => {
      ...
      const itemIndex = [...menuItems].findIndex((menuItem) => item === menuItem);
    ...
    () => ({
      ...
      getMenuListProps: (_props = {}, _ref = null) => ({
        ...
        onBlur: (event) => {
          ...
          const targetIsMenu =
            menuListRef.current === relatedTarget || menuListRef.current?.contains(relatedTarget);
          const targetIsTrigger =
            triggerRef.current === relatedTarget ||
            triggerRef.current?.contains(relatedTarget) ||
            isMouseDown.current;
          const targetIsSubmenu = relatedTarget?.parentElement?.dataset.parentMenu === menuId;
