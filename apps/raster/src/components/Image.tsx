...
export default function RasterImage({ image, original, selected, displayName, chooseImage, openVersions, thumbnail, version, versionSelected }: Props) {
    ...
    <div className="relative flex flex-col gap-1.5">
      {chooseImage && (
        <button
          ...
          key={image.id}
          className={clsx('relative group image-button', selected ? 'border-primary' : 'border-white', thumbnail ? 'cursor-move' : 'overflow-hidden', {
            'h-full': version,
          })}>
          ...
          <span className={clsx('checkmark', selected && !thumbnail ? 'scale-100 opacity-100' : 'scale-90 opacity-0')}>
            <Check />
          </span>
